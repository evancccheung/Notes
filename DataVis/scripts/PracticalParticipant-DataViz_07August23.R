...
require(scales) # nicer axis scale labels
# install.packages("gapminder")
library(gapminder)
if(!require(gapminder)) {install.packages("gapminder", quietly=T); library(gapminder)} # install the package if it wasn't

str(gapminder)
head(gapminder)
summary(gapminder)
with(gapminder, table(continent, year))
with(gapminder, round(prop.table(table(continent, year)), 3))
...
ggplot(gapminder, aes(x=continent, fill=continent)) + 
  geom_bar(aes(y=after_stat(count)/12)) + # transform the count to count the no. of countries
...
ggsave("fig_output/mybar.png", dpi = 200)
...
ggsave("fig_output/lifeExp_continent.png", dpi = 200)

ggplot(data=gapminder, aes(x=lifeExp)) + 
  geom_histogram(aes(y=..density..), binwidth=4, color="black", fill="lightblue", alpha=0.5) # using histogram

ggsave("fig_output/lifeExp_histogram.png", dpi = 200)
...
ggsave("fig_output/lifeExp_continent_box.png", dpi = 200)

#*******YOUR TURN**********
#1. Remove the legend from this plot
gap1 + geom_boxplot(outlier.size=1.5) + guides(fill="none")

#2. Also, make the plot horizontal
gap1 + geom_boxplot(outlier.size=1.5) + guides(fill="none") + coord_flip()

#3. Instead of a boxplot, try geom_violin()
gap1 + geom_violin() + guides(fill="none") + coord_flip()

#4. Change it to horizontal density plot for each continent
require(ggridges)
ggplot(data=gapminder) + aes(x=lifeExp, y=continent, fill=continent) + 
  geom_jitter(alpha=0.3, width=0.6, aes(color=continent)) +
  geom_density_ridges(alpha=0.5) + guides(fill="none") +
  xlab("Life Expentancy") + ylab("Continents")
...
gapminder %>% #piping the result of this right into ggplot
  ...
  geom_boxplot(outlier.size=2) + guides(fill="none")
...
ggplot(data=gapminder) + aes(x=gdpPercap, fill=continent) + 
  geom_density(alpha=0.5) + facet_wrap(~continent, ncol=1) + guides(fill="none") +
  xlab("log(GDP)") + ylab("Continents") 
...
ggplot(data=gapminder) + aes(x=continent, y=gdpPercap, fill=continent) +
  geom_boxplot(outlier.size=1.5, alpha=0.4) + guides(fill="none") +
  xlab("Continents") + ylab("log(GDP)")

#4. Do the same, but plot GDP on a log scale
ggplot(data=gapminder) + aes(x=continent, y=log10(gdpPercap), fill=continent) +
  geom_boxplot(outlier.size=1.5, alpha=0.4) + guides(fill="none") +
  xlab("Continents") + ylab("log(GDP)")
...
ggplot(subset(gapminder, country == "India" | country == "China")) +
  aes(x=gdpPercap, y=lifeExp, color=country) + geom_line() + geom_point() +
labs(title = "Life Expectancy vs GDP per Capita for China & India",
...
ggplot(subset(gapminder, country == "India" | country == "China")) + 
  ...
  labs(title = "Change in life expectancy over time for China & India ",
...
gapminder %>% #piping to ggplot
  ...
  geom_line() + 
  geom_point(size=1.5) + 
  labs(title="Change in life Expectancy over Time for Different Continents",
       x="Year", y="Life Expectancy")
...
ggplot(gapyear, aes(x=year, y=lifeExp, color=continent)) +  #fitting linear regression lines for each continent
  geom_point(size=1.5) +
  geom_smooth(aes(fill=continent), method="lm", linewidth=0.7) +
  labs(title="Change in life Expectancy over Time for Different Continents",
       x="Year", y="Life Expectancy")

ggplot(gapyear, aes(x=year, y=lifeExp, color=continent)) +  #using a loess smooth
  geom_point(size=1.5) +
  geom_smooth(aes(fill=continent), method="loess", linewidth=0.7) + 
  labs(title="Change in life Expectancy over Time for Different Continents",
       x="Year", y="Life Expectancy")
